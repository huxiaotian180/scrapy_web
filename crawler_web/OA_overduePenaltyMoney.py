...
def login(count):
    ...
    while count < 5:
        try:
            ...
            browser.find_element_by_id("password").send_keys('xxxxxxxx@yixin')
...
# 获取查询数据
def select_data():
    if not os.path.exists(r'X:/yx/overdue/overdue_list.txt'):
        sys.exit(0)

    with open(r'X:/yx/overdue/overdue_list.txt') as f:
...
def get_yixin_data(cookies):
    ...
    if os.path.exists(targetPath):
        f = open(r'X:/yx/overdue/overduePenalty.txt', 'w', encoding="utf-8")
        info_text = "申请编号,当期期数,月供,本金,利息,贷款余额,实际还款日期,计划还款日期,还款状态,逾期天数,罚息\n"
        f.write(info_text)
        print("开始写入文件")
    else:
        ...
        try:
            ...
            if len(text_message) > 0:
                for data in text_message:
                    if str(data["repayStateName"]).strip() == "已逾期":
