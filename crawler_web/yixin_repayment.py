...
#获取验证码登录验证码
def cutcode_login(time_start,login_integral,count):
    if time_start>=80000 and time_start<=235959 and login_integral>0:
        flag = True
        while (flag and count<5):
            try:
                ...
                driver.find_element_by_id('txt_Password').send_keys('xxxxx@yixin')
                driver.implicitly_wait(30)
...
def time_info():
    ...
    time_day = time_list[1].split(":")
    time_str = time_day[0] + time_day[1] + time_day[2]
...
if __name__ == "__main__":
    ...
    login_integral = get_integral()
    cutcode_login(time_start,login_integral,count)
    get_jifen()