        ...
        if self.ret_code == 0:
            ...
            if rslt_data is not None and rslt_data != "":
                ...
                if "cust_val" in jrsp_ext:
                    data_code        = jrsp_ext["cust_val"];
                    self.cust_val   = float(data_code);
                if "result" in jrsp_ext:
                    data_code        = jrsp_ext["result"];
                    self.pred_rsp.value     = data_code;
...
def TestFunc():
    pd_id           = "106335";     #用户信息页可以查询到pd信息
    pd_key          = "YgAAxcNHF5lnR3qLbKQgoSI5swdOGo3e";
    ...
    file_name       = r'C:\python\verifycodeimage.png';
