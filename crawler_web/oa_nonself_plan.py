...
def login(count):
    ...
    while count < 5 and flag:
        try:
            ...
            browser.find_element_by_id("username").send_keys('xxxxxx')
...
def select_data():
    try:
        if not os.path.exists(r'X:/yx/non-self-plan/to-do-list/yx_repay_plan.txt'):
            sys.exit(0)

        with open(r'X:/yx/non-self-plan/to-do-list/yx_repay_plan.txt') as f:
...
def get_yixin_data(cookies):
    # 准备写入文件数据
    targetPath = 'X:/yx/non-self-plan/down-list'
    print(os.path.exists(targetPath))
    if os.path.exists(targetPath):
        f = open(r'X:/yx/non-self-plan/down-list/OA_REPAY_PLAN.txt', 'w', encoding="utf-8")
        info_text = "申请编号,当期期数,月供,本金,利息,贷款余额,计划还款日期\n"
        f.write(info_text)
        print("开始写入文件")
    # else:
        ...
        try:
            ...
            if len(text_message) > 0:
                for data in text_message:
                    if data["currentRepayPeriod"] > 0:
                        info_message = "%s,%s,%s,%s,%s,%s,%s\n" % (
                            line,
